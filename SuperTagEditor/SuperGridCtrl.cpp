		...
		case VK_UP:
			if (GetAsyncKeyState(VK_MENU) & 0x8000) {
				...
				if(nIndex > 1)
				{
					...
					/*
					UINT uflag = LVIS_SELECTED | LVIS_FOCUSED;
					SetItemState(nIndex, GetItemState(nIndex-1, LVIS_SELECTED), uflag);
					SetItemState(nIndex-1, uflag, uflag);
					UpdateData(pItem, lpPrev);
					UpdateData(pPrevItem, lp);
					LV_ITEM lvItem;
					lvItem.mask = LVIF_TEXT | LVIF_INDENT | LVIF_PARAM;
					CString strItem = lpPrev->GetItemText();
					lvItem.pszText = strItem.GetBuffer(1);
					lvItem.iItem = nIndex;
					lvItem.lParam = (LPARAM)pPrevItem;
					lvItem.iIndent = GetIndent(pPrevItem);
					lvItem.iSubItem = 0;
					CListCtrl::SetItem(&lvItem);
					int nSize = lp->GetItemCount();
					for(int i=0; i < nSize;i++)
					{
					   CString str = lpPrev->GetSubItem(i);
		...
		case VK_DOWN:
			if (GetAsyncKeyState(VK_MENU) & 0x8000) {
				...
				if(nIndex > 0 && GetItemCount() > (nIndex+1)) {
					...
					SetItemState(nIndex, GetItemState(nIndex-1, LVIS_SELECTED), uflag);
					SetItemState(nIndex+1, uflag, uflag);
					UpdateData(pItem, lpNext);
					UpdateData(pNextItem, lp);
					LV_ITEM lvItem;
					lvItem.mask = LVIF_TEXT | LVIF_INDENT | LVIF_PARAM;
					CString strItem = lpNext->GetItemText();
					lvItem.pszText = strItem.GetBuffer(1);
					lvItem.iItem = nIndex;
					lvItem.lParam = (LPARAM)pNextItem;
					lvItem.iIndent = GetIndent(pNextItem);
					lvItem.iSubItem = 0;
					CListCtrl::SetItem(&lvItem);
					int nSize = lp->GetItemCount();
					for(int i=0; i < nSize;i++)
					{
					   CString str = lpNext->GetSubItem(i);
					   lvItem.mask = LVIF_TEXT;
					   lvItem.iSubItem = i+1;
					   lvItem.pszText = str.GetBuffer(1);
					   SetItem(&lvItem);
...
#ifdef FLICKERFREE
#if 0 // http://bbs1.parks.jp/22/hasetaS/bbs.cgi?Action=Res&Mode=Tree&Base=71&Fx=0
		LONG style = GetWindowLong(GetSafeHwnd(), GWL_STYLE);
		if (m_bMultiSelectLine) {
			style &= ~LVS_SINGLESEL;
		} else {
			style |= LVS_SINGLESEL;
		}

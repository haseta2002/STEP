...
static TCHAR *StrCpy(TCHAR *ret, const TCHAR *buf);
...
BOOL Profile_Flush(const TCHAR *path)
{
	...
	for(i = 0; i < SectionCnt; i++){
		...
		if(i != 0){
			*(p++) = TEXT('[');
			p = StrCpy(p, (SectionInfo + i)->SectionName);
			p = StrCpy(p, TEXT("]\r\n"));
		}
		...
		for(j = 0; j < (SectionInfo + i)->KeyCnt; j++){
			...
			//キー名
			p = StrCpy(p, ((SectionInfo + i)->KeyInfo + j)->KeyName);
			if(((SectionInfo + i)->KeyInfo + j)->CommentFlag == FALSE){
				*(p++) = TEXT('=');

				if(((SectionInfo + i)->KeyInfo + j)->String != NULL){
					//文字列
					p = StrCpy(p, ((SectionInfo + i)->KeyInfo + j)->String);
				}
			}
			p = StrCpy(p, TEXT("\r\n"));
		}

...
#if 0
		g_rectMainWindow.left	= (int)wndpl.rcNormalPosition.left;
		g_rectMainWindow.right	= (int)wndpl.rcNormalPosition.right;
		g_rectMainWindow.top	= (int)wndpl.rcNormalPosition.top;
		g_rectMainWindow.bottom	= (int)wndpl.rcNormalPosition.bottom;
#else
...
void CSuperTagEditorView::OnDropFiles(HDROP hDropInfo) 
{
	...
	for (int i = 0; i < nFileCount; i++) {
		...
		CFileStatus	status;
/* STEP 044 *///		if (CFile::GetStatus(sFileName, status) == TRUE && status.m_mtime != -1) {
		if (CFile::GetStatus(sFileName, status) == TRUE) {
...
void CSuperTagEditorView::ExecWinampCommand(int nCommand, LPARAM lParam)
{
	...
	case CONTROL_WINAMP_PLAYFILE:
		if (g_nOptPlayerType == PLAYER_WINAMP) {
			...
			for (int i = 0; i < (int)cds->cbData; i++) {
...
void CSuperTagEditorView::OnConvFormatUser() /* AstralCircle 041 */
{
	...
	for (int i = 0; i < USER_CONV_FORMAT_MAX; i++) {
...
void CSuperTagEditorView::OnConvExSetup() 
{
	...
	for (int i = 0; i < USER_CONV_FORMAT_EX_MAX; i++) {
...
void CSuperTagEditorView::OnConvTag2tagSetup() 
{
	...
	for (int i = 0; i < USER_CONV_FORMAT_TAG2TAG_MAX; i++) {

...
LPCTSTR COXToolTipCtrl::m_szArrowSpace = _T("Å@");
...
void COXToolTipCtrl::OnPaint() 
{
    ...
    CString str;
	if (m_bExtended) {
        str = /*m_szArrowSpace +*/ GetFieldFromString(m_pCurrentToolTip->m_strTipText, 1, _T('\r'));
	} else {
		str = m_pCurrentToolTip->m_strTipText;//GetTooltipText(m_pCurrentToolTip);
	}
    if (str.IsEmpty())
        return;
...
void COXToolTipCtrl::RelayEvent(MSG* pMsg)
{
    ...
    if (pMsg->message == WM_MOUSEMOVE)
    {
		...
		//if (pToolTip) {
		//	CString strTip;
		//	strTip = GetTooltipText(pToolTip);
		//	TRACE1("COXToolTipCtrl::RelayEvent1 [%s]\n", strTip);
		//}
		/***
		if (pToolTip && m_pCurrentToolTip && m_pCurrentToolTip->m_strTipText == pToolTip->m_strTipText) {
			TRACE1("COXToolTipCtrl::RelayEvent2 [%s]\n", m_pCurrentToolTip->m_strTipText);
			return;//m_pCurrentToolTip = NULL;
		}
		if (pToolTip && m_pCurrentToolTip && m_pCurrentToolTip->m_strTipText != pToolTip->m_strTipText) {
			m_pCurrentToolTip = NULL;
		}

...
CToolBarInfo CMainFrame::mainToolBar[] =
{
	...
	{{20, ID_TEIKEI , TBSTATE_ENABLED, TBSTYLE_BUTTON | TBSTYLE_DROPDOWN, 0, 26}, "定型文貼り付け"},
...
//	}
	...
	{ /* AstralCircle 041 */
		...
		dwStyle = m_wndToolBar.GetButtonStyle(m_wndToolBar.CommandToIndex(ID_TEIKEI));
		dwStyle |= TBSTYLE_DROPDOWN;
		m_wndToolBar.SetButtonStyle(m_wndToolBar.CommandToIndex(ID_TEIKEI), dwStyle);
		// StartInaction 053 */
	...
	CCreateContext* pContext)
{
	...
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CLeftView), CSize(g_nSplitSize, 100), pContext) ||
		!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CSuperTagEditorView), CSize(100, 100), pContext))
	{
...
void CMainFrame::OnToolbarDropDown(NMTOOLBAR* pnmtb, LRESULT *plr) /* AstralCircle 041 */
{
...
BOOL CMainFrame::OnToolTipNotify( UINT id, NMHDR * pNMHDR, LRESULT * pResult )
{
	...
	extern bool OnToolTipNotify(UINT nID, LPCTSTR& szText);

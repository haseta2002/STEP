...
//static void WriteKbSTEPathToRegistry(void);
//static void WriteKbSTEPathToRegistry(void)
//{// KbSTE.EXE のフルパスのファイル名を
 //
 // HKEY_CURRENT_USER\Software\Kobarin's Soft\KbSTE\KbSTEPath
 //
 // に書き込む
 // 
 // KbMedia Player Version 2.07β4 以降は起動時にこの値を調べ、
 // 値が有効なら KbSTE との連携を行えるようにする
 // （Media エクスプローラから KbSTE を起動できるようにする）
/*
    HKEY hKey = NULL;
    DWORD Disposition = 0;
    RegCreateKeyEx(HKEY_CURRENT_USER, 
                   "Software\\Kobarin's Soft\\KbSTE", 
                   0, 
                   NULL,
...
CString		g_strPluginIniFileName; /* 2006.06.19 */
...
BOOL CSuperTagEditorApp::InitInstance()
{
	...
	{ /* STEP 031 */
		...
		for (int i = 1; i < __argc; i++) {
			...
			if (bFlag) {
				free((void *)m_pszProfileName);
				m_pszProfileName = strdup(pszParam);
				break;
			}
...
CAboutDlg::CAboutDlg() : CDialog(CAboutDlg::IDD)
{
	...
	m_strVersion.Format(PROG_NAME_ORG" Ver "PROG_VERSION_ORG"(Build:%d) 改", BUILDCOUNT_NUM);
}
...
static	char	*sKeyPluginIniFileName = "PluginIniFileName"; /* 2006.06.19 */
...
void CSuperTagEditorApp::ReadRegistry(void)
{
	...
	{ /* 2006.06.19 */
		CString		strSectionName;
		strSectionName = "expert";
		g_strPluginIniFileName = MyGetProfileString(strSectionName, sKeyPluginIniFileName, "");
	}
	//Profile_Free();
}

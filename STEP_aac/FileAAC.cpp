...
bool LoadFileAAC(FILE_INFO *pFile)
{
	...
	if (MP4GetMetadataGroup(h, &value) == true) { /* Žæ“¾‚Å‚«‚é‚æ‚¤‚Émp4v2.dll‚É’Ç‰Á */
		if (UTF8toSJIS(value, &buff) == true) {
			SetKeywordSI(pFile, buff);
			free(buff);
		}
	}
	if (MP4GetMetadataAlbumArtist(h, &value) == true) { /* Žæ“¾‚Å‚«‚é‚æ‚¤‚Émp4v2.dll‚É’Ç‰Á */
		if (UTF8toSJIS(value, &buff) == true) {
...
bool WriteFileAAC(FILE_INFO *pFile)
{
	...
	*/
	...
	while (MP4GetMetadataByIndex(h,index, &name, &value, &size) == true) {
		if (!stricmp(name, "trkn") || !stricmp(name, "tracknumber") ||
			...
			!stricmp(name, "©nam") ||
			!stricmp(name, "©alb") ||
			!stricmp(name, "©day") || !stricmp(name, "year") ||
			!stricmp(name, "©cmt") ||
			!stricmp(name, "©too") ||
			!stricmp(name, "©grp") ||
			!stricmp(name, "aART") ||
	...
	*/
	...
	for (int i=0;i<tag.GetSize();i++) {
		...
		/*
		else if (!stricmp(meta->name, "©ART"))
		else if (!stricmp(meta->name, "©wrt"))
		else if (!stricmp(meta->name, "©nam"))
		else if (!stricmp(meta->name, "©alb"))
		else if (!stricmp(meta->name, "©day") || !stricmp(meta->name, "year"))
		else if (!stricmp(meta->name, "©cmt"))
		else if (!stricmp(meta->name, "©gen"))
		else if (!stricmp(meta->name, "©too"))
		else if (!stricmp(meta->name, "©grp"))
		else if (!stricmp(meta->name, "©gen"))
		*/

...
//	if (hFindFile != INVALID_HANDLE_VALUE) FindClose(hFindFile); /* 2004.02.18 上に移動 */
...
#if 0
/*
void CSuperTagEditorDoc::OnUpdateDlgEnvironmentNormal(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(TRUE);
}
void CSuperTagEditorDoc::OnDlgEnvironmentNormal()
{
	CDlgEnvSheet	dialog("オプション設定");
	bool	bKeepTimeStamp = g_bOptKeepTimeStamp;
	bool	bHideMP3ListFile = g_bOptHideMP3ListFile;
	...
	dialog.m_pDoc = this;
	dialog.DoModal(CDlgEnvSheet::ENV_NORMAL);
...
// [オプション設定]-[変換]
/*
void CSuperTagEditorDoc::OnUpdateDlgEnvironmentConv(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(TRUE);
}
void CSuperTagEditorDoc::OnDlgEnvironmentConv()
{
	CDlgEnvSheet	dialog("オプション設定(変換)");

	dialog.m_pDoc = this;
	dialog.DoModal(CDlgEnvSheet::ENV_CONV);
}
*/

// [オプション設定]-[上級者向け]
/*
void CSuperTagEditorDoc::OnUpdateDlgEnvironmentAdvanced(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(TRUE);
}
void CSuperTagEditorDoc::OnDlgEnvironmentAdvanced()
{
	CDlgEnvSheet	dialog("オプション設定(上級者向け)");

	dialog.m_pDoc = this;
	dialog.DoModal(CDlgEnvSheet::ENV_ADVANCED);
	...
	for (int no = 0; no < m_nArrayFileCount; no++) {
		...
		if (fileMP3->nFormat != FILE_FORMAT_UNKNOWN && fileMP3->bModifyFlag) {
			if (CFileMP3::WriteTag(fileMP3, g_bOptKeepTimeStamp)) {
				...
				MessageBox(NULL, "タグ情報を更新できませんでした", fileMP3->strFullPathName, MB_ICONSTOP|MB_OK|MB_TOPMOST);
				m_bSaveAllTagResult = false;
			}
		}
	}

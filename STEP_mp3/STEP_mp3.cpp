...
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	return "Version 1.03 Copyright (C) 2003-2010 haseta\r\nMP3(ID3v1/ID3v2)/RIFF形式をサポートしています";
}
...
//	if (strlen(GetGenreSI(pFileMP3)) == 0 && GetBGenre(pFileMP3) != 0xFF) {
//		SetGenreSI(pFileMP3, STEPGetGenreNameSIF(GetBGenre(pFileMP3)));
//	}
	SetTrackNumberSI(pFileMP3, id3v2.GetTrackNo());		// トラック番号
//	if (strlen(GetTrackNumber(pFileMP3)) == 0 && GetBTrackNumber(pFileMP3) != 0xff) {
//		CString strTrackNumber;
//		strTrackNumber.Format("%d", GetBTrackNumber(pFileMP3));
//		SetTrackNumber(pFileMP3, strTrackNumber);
//	}
//	if (!strlen(GetTrackNumber(pFileMP3)) == 0 && GetBTrackNumber(pFileMP3) == 0xff) {
//		SetBTrackNumber(pFileMP3, (BYTE)atoi(CId3tagv2::GetIntTrackNo(GetTrackNumber(pFileMP3))));
//	}
	...
	SetAlbumArtistSI(pFileMP3, id3v2.GetAlbumArtist());	// アルバムアーティスト
...
#undef LIMIT_TEXT_LENGTH
	...
	if (strlen(GetTrackNumberSI(pFileMP3)) == 0/* && GetBTrackNumber(pFileMP3) != (BYTE)0xff*/) { /* 未設定時のみ */
		/*
		CString strTrackNumber;
		strTrackNumber.Format("%d", GetBTrackNumber(pFileMP3));
...
//	}
	...
	id3v2.SetAlbumArtist(GetAlbumArtistSI(pFileMP3));	// アルバムアーティスト
...
bool IsCreateID3v2(FILE_INFO *pFileMP3)
{
	...
	if (GetBTrackNumber(pFileMP3)		!= 0xff) { /* トラック番号によってコメントのサイズを変える 2006.09.27 */
		if (strlen(GetComment(pFileMP3))	> ID3_LEN_COMMENT-2)	return true;
	} else {
		if (strlen(GetComment(pFileMP3))	> ID3_LEN_COMMENT)	return true;
	}
...
bool IsCreateID3v2SI(FILE_INFO *pFileMP3, bool bID3v1Only = FALSE)
{
	...
	if (strlen(GetSoftwareSI(pFileMP3)) > 0
		&& strcmp(GetSoftwareSI(pFileMP3), strOptSoftwareTag) != 0)		return true;	// ソフトウェア
	if (strlen(GetAlbumArtistSI(pFileMP3)) > 0)						return true;	// アルバムアーティスト /* STEP 042 */
	...
	if (strlen(GetCompilationSI(pFileMP3)) > 0)						return true;	// コンピレーション /* STEP 049 */
	return false;
}
	...
	if (nFormat == nFileTypeMP3 || nFormat == nFileTypeMP3V1 || nFormat == nFileTypeMP3V11) {
		...
		if (bOptAutoConvID3v2) {
			if (IsCreateID3v2(pFileMP3) || IsCreateID3v2SI(pFileMP3)/* || !IsMatchID3SI(pFileMP3)*/) {
				if (bOptID3v2ID3tagAutoWrite) { /* 2006.09.27 */
					// ID3tag自動更新の場合はSIに設定されるのでコピー
					copySIField(pFileMP3);
				}
				...
				if (bOptID3v2ID3tagAutoWrite) { /* 2006.09.27 ID3tagで入力したものが消えるので */
					// ID3tag自動更新の場合はSIに設定されるのでコピー
					copySIField(pFileMP3);
				}
			}
			...
			if (IsCreateRMP(pFileMP3) /*|| !IsMatchID3SI(pFileMP3)*/) {
				...
				if (bOptRmpID3tagAutoWrite) { /* 2006.09.27 */
					// ID3tag自動更新の場合はSIに設定されるのでコピー
					copySIField(pFileMP3);
				}
			}

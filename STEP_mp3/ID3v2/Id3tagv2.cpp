...
void CId3tagv2::_SetStringEncode(int encode)
{
	...
	while(p != m_frames.end())
	{
		...
		/* STEP */
		// SetId3String()で文字コード変換が行われるフレームを対象にする
		// APICフレームの文字コードをUTF16にするとMp3tagで画像が読み込めなくなるため
		// おそらく Description の終端文字が $00 $00 ではなく $00 であるため
		// 現状では TEXTフレーム/URLフレーム/コメントフレーム のみだが
		// フレームIDごとに変換をおこなうべきか？Setter/GetterがないフレームIDの扱いは？
		DWORD id = pFrame->GetId();
		char szId[sizeof(id)+1];
		memcpy(szId,&id,sizeof(id));
		switch(szId[0]){
			case 'T':
			case 'W':
			case 'C':
		...
		szId[sizeof(id)] = 0;
		CString info = GetId3String(szId);
		
		if(strlen(info) != 0) { // SetId3String()で削除されるので /* STEP */
			SetId3String(szId, info);
		}

		unsigned char *data = pFrame->GetData();
		if (pFrame->GetSize() || data == NULL) { /* STEP */
			p++;
			continue;
		}
...
CString CId3tagv2::GetGenre()
{
	...
	if (strGenre.IsEmpty()) { /* STEP */
		while(1)
		{
			if(strBracket.GetLength() &&
				!IsDBCSLeadByte(strBracket[0]) &&
				(strBracket[0] == '(') )	//頭にカッコを検出
			{
				strBracket.Delete(0);	//先頭の'('を削除
				int index = strBracket.Find(')');
				if(0 <= index)
				{
					strBracket.Delete(index,index);	//最後の')'を削除
					continue;
				}
			}
			break;
		}
		unsigned char cGenre = -1;
		if (strBracket == "0") {
			cGenre = 0;
		} else {
			cGenre = atoi(strBracket);
			if (cGenre == 0) {
				cGenre = -1;
			}
		}
		if (cGenre >= 0) {
			CId3tagv1 id3tagv1(TRUE);
			strGenre = id3tagv1.GenreNum2String(cGenre);
		}
	}
	return strGenre;
}
...
DWORD CId3tagv2::Load(const char *szFileName)
{
	...
	while(dwRemainSize)
	{
		...
		{
			/* STEP */
			// TEXTフレームのみエンコードのチェックを行う
			// CId3tagv2::_SetStringEncode()に施した対応により複数のエンコードが混在することがあるので
			DWORD id = frame.GetId();
			char szId[sizeof(id)+1];
			memcpy(szId,&id,sizeof(id));
			switch(szId[0]){
				case 'T':
...
/* STEP では非同期化は必要なときのみ行うので、サイズでフラグのON/OFFを決定する */
		if(dwEncodeSize != dwTotalFrameSize) /* STEP */
		{
			//非同期化フラグをセット
			m_head.flag |= 0x80;
...
/*==================== STE改で追加 ======================*/
#if 0
CString CId3tagv2::GetIntTrackNo(const CString &no) /* TunamiStruck 003 */
{
	//トラック番号
	if (IsNumeric(no)) {
		return no;
...
bool CId3tagv2::IsNumeric(const CString &str) /* TunamiStruck 003 */
{
	if (str.IsEmpty()) {
...
void CId3tagv2::SetGenre(const char *szGenre, bool addNumber) /* SharkAttack 016 */
{
	//ジャンル
...
/*
			return CString((LPCSTR )&data[1],p->second.GetSize()-2);
*/
/*
extern	bool		g_bId3v2OldComment;	/* FunnyCorn 180 */

/*
	m_bUnicodeEncode = g_bId3v2UNICODE; /* FunnyCorn 181 */
/*
extern bool		bOptId3v2OldComment;
extern bool		bOptId3v2UNICODE;
extern bool		bOptUnSyncAlways;
*/

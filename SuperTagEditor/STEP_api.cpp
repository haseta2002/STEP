...
PSTEPlugin STEPluginLoadFile(LPCTSTR strPluginFile) {
	...
	CString strPluginFolder;
	{
		...
		_tmakepath(buff, drive, dir, NULL, NULL);
		if (drive[0] == '\0') {
...
void STEPluginLoad(HWND hWnd) {
	...
	CString strINI;
	{
		...
		if (g_strPluginIniFileName.GetLength() == 0) { /* 2006.06.19 */
		} else {
			_tmakepath(buff, drive, dir, g_strPluginIniFileName, "");
		}
		strINI = buff;
		BOOL isExists = Profile_Initialize(strINI, TRUE);
		Profile_Free();
		if (!isExists) {
			_tmakepath(buff, drive, dir, "DefaultPlugin", "ini");
			strINI = buff;
		}
		delete szName;
	}
...
{
	for (int i=0;i<arFormatType.GetSize();i++) {
...
int GetFileImageIndex(UINT nFormat, bool bModify)
{
	...
	for (int i=0;i<pPlugin->arExtInfo.GetSize();i++) {
...
bool CheckFileExt(const FILE_MP3* pFileMP3, const char* ext)
{
	...
	for (int j=0;j<pPlugin->arExtInfo.GetSize();j++) {
		PSTEPExtInfo info = (PSTEPExtInfo)pPlugin->arExtInfo.GetAt(j);
		if (strcmpi(ext, info->strExt) == 0) {
			return true;
		}
		if (ext[0] == '.' && strcmpi(ext+1, info->strExt) == 0) {
			return true;
		}
	}
	return false;
...
void ConvSIFieldToID3tag(FILE_MP3* pFileMP3) /* STEP 037 */
{
	...
	for (int i=	COLUMN_TRACK_NAME;i<COLUMN_MAX;i++) {
...
extern "C" STEP_API LPCTSTR WINAPI STEPGetValue(FILE_INFO* pFileInfo, FIELDTYPE nField)
{
	...
	case FILED_WRITER_SI:				// 作詞者 /* STEP 043 */
		return pFileMP3->strWriter;
	case FIELD_DISK_NUMBER_SI:			// ディスク番号 /* STEP 045 */

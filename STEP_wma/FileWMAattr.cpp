...
//			pFileMP3->nBitRate = atoi(strData) / 1000;
		...
		} else if (strcmpi(strName, "WM/Year") == 0) {
			// リリース
			if (bAppend) {
		...
		} else if (strcmpi(strName, "WM/AlbumTitle") == 0) {
			// アルバム
			if (bAppend) {
		...
		} else if (strcmpi(strName, "Author") == 0) {
			// アーティスト
			if (bAppend) {
		...
		} else if (strcmpi(strName, "Title") == 0) {
			// タイトル
			if (bAppend) {
		...
		} else if (strcmpi(strName, "Description") == 0) {
			// コメント
			if (bAppend) {
		...
		} else if (strcmpi(strName, "WM/Genre") == 0) {
			// ジャンル
			if (bAppend) {
		...
		} else if (strcmpi(strName, "WM/GenreID") == 0) {
			// ジャンル
			if (bAppend) {
				strPrev = GetGenreSI(pFileMP3);
				strData = strPrev + ";" + strData;

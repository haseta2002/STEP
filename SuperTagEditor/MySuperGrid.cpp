...
// 	{"BitRate"   , FALSE, 27, 128, 0, LVCFMT_LEFT },	// ビットレート /* Conspiracy 198 */
	...
	{"FileCTime" , FALSE, 29, 128, 128, LVCFMT_LEFT },	// ファイル作成日 /* 2003.06.19 add */
	{"AlbumArtist" , FALSE, 30, 128, 128, LVCFMT_LEFT },	// アルバムアーティスト /* STEP 042 */
	{"Writer"    , FALSE, 31, 128, 128, LVCFMT_LEFT },	// 作詞者 /* STEP 043 */
	{"DiskNo"    , FALSE, 32, 48, 48, LVCFMT_RIGHT },	// ディスク番号 /* STEP 045 */
	{"Compilation",FALSE, 33, 48, 48, LVCFMT_LEFT },	// コンピレーション /* STEP 049 */
};
...
歌詞ファイル, ファイルの種類, フォーマット, その他, アルバムアーティスト
*/
...
{
	CString	strBuffer;
	if (strnicmp(strFullPath, strBaseDir, strBaseDir.GetLength()) == 0) {
		// 相対パスに変換可能(パスが一致)
		strBuffer.Format("%s", strFullPath.Right(strFullPath.GetLength()-strBaseDir.GetLength()));
	} else if (strFullPath[1] == ':' && strnicmp(strFullPath, strBaseDir, 1) == 0) {
		// 相対パスに変換可能(ドライブ名だけ一緒)
...
//			controlTable[COLUMN_FILE_NAME] = GetControlType(nFormat, COLUMN_FILE_NAME, isEditSIF);
			...
			controlTable[COLUMN_DISK_NUMBER] = GetControlType(nFormat, COLUMN_DISK_NUMBER, isEditSIF); /* STEP 045 */
...
//			colMax[COLUMN_FILE_NAME] = GetColumnMax(nFormat, COLUMN_FILE_NAME, isEditSIF);
			...
			colMax[COLUMN_DISK_NUMBER] = GetColumnMax(nFormat, COLUMN_DISK_NUMBER, isEditSIF); /* STEP 045 */
			...
			*/
			if (i%2 == 0) {
				...
				if (HasOriginalColumnName(nFormat) == true) {
					...
					nameList[COLUMN_DISK_NUMBER] = GetColumnName(nFormat, COLUMN_DISK_NUMBER); /* STEP 045 */
...
void CMySuperGrid::InitializeGrid()
{
	...
	*/
	...
	for (i = 0; i < COLUMN_MAX; i++) {
		COLUMN_STATUS	*stat = &g_columnStatus[i];
...
bool CMySuperGrid::AddFile(const FILE_MP3 *fileMP3, CTreeItem *pItemParent, LPARAM lParam, bool bCheck)
{
	...
	while (pTotalParent != NULL) { /* TunamiStruck 007 */
		...
		if (!g_bOptShowTotalParent)	break;
...
CString CMySuperGrid::GetFileColumnText(const FILE_MP3 *fileMP3, int nColumn)
{
	...
	case COLUMN_TRACK_NAME:		// トラック名
		...
		//return(fileMP3->strTrackNameSI.IsEmpty() ? fileMP3->strTrackName : fileMP3->strTrackNameSI);
	case COLUMN_ARTIST_NAME:	// アーティスト名
		...
		//return(fileMP3->strArtistNameSI.IsEmpty() ? fileMP3->strArtistName : fileMP3->strArtistNameSI);
	case COLUMN_ALBUM_NAME:		// アルバム名
		...
		//return(fileMP3->strAlbumNameSI.IsEmpty() ? fileMP3->strAlbumName : fileMP3->strAlbumNameSI);
	case COLUMN_TRACK_NUMBER:	// トラック番号
		...
		//if (fileMP3->byTrackNumber == 0xff) { /* BeachMonster 097 */
		//	strBuffer.Empty();
		//} else {
		//	strBuffer.Format("%d", fileMP3->byTrackNumber);
		//}
		//if (nReadType == READ_ID3 && !IsSupportTrackNumberSIF/*IsEditTrackNumberSIF*/(fileMP3)/* Conspiracy3 202 && !g_bOptEditFieldSIF*/) {
		//} else
		//	return(fileMP3->strTrackNumber); /* Conspiracy3 202 */
			/* Conspiracy3 202 *///return(fileMP3->strTrackNumber.IsEmpty() ? strBuffer : fileMP3->strTrackNumber); /* WildCherry 063 */
		/* 問題があるかな～
		if (fileMP3->nFormat == FILE_FORMAT_ID3V2 && nReadType == READ_SIF) {
			return(fileMP3->strTrackNumber);
		}
	...
	case COLUMN_GENRE:			// ジャンル番号
		...
		// ID3v1とSI/ID3v2で個別に表示
		// g_bOptEditFieldSIF を見てるのはID3v2/RIFFの任意ジャンルが入力された場合を考慮
		//if (nReadType == READ_ID3 && !g_bOptEditFieldSIF) return(fileMP3->strGenre); /* BeachMonster 093 */
		//return(fileMP3->strGenreSI.IsEmpty() ? fileMP3->strGenre : fileMP3->strGenreSI); /* BeachMonster 093 */
		//return(fileMP3->strGenre); /* BeachMonster 093 */
...
//			strBuffer = fileMP3->strFileName;
	...
	// ファイル作成日を変更不可にする
	SetColumnControlType(pInfo, invalid, COLUMN_FILE_CTIME); /* STEP 2008.05.02 */
	// ファイルパス名を変更不可にする
	...
	for (int nItem = sy; nItem <= ey; nItem++) {
		...
		if (pItem != NULL && IsItemFile(pItem) == TRUE) {
			...
			for (int nColumn = sx; nColumn <= ex; nColumn++) {
				...
				strBufferText += str; /* RockDance 124 */ /* quoteForComment()する前に設定 2007.04.03 */
				...
				if (pItem != NULL && IsItemFile(pItem) == TRUE) {
					...
					for (int nColumn = sx; nColumn <= ex; nColumn++) {
						...
						if (bText) { /* RockDance 124 */
							ChangeSubItemText(nItem, nColumn, strBuffer, nPastePos);
						} else {
							ChangeSubItemText(nItem, nColumn, unquoteForComment(strBuffer), nPastePos);
						}
#ifdef FLICKERFREE
						InvalidateItemRect(nItem);
#endif
		...
		if (pItem != NULL && IsItemFile(pItem) == TRUE) {
			...
			if ((pStr = GetToken(pLineTop, "\t")) != NULL) {
				for (int nColumn = sx; nColumn <= ex; nColumn++) {
					...
					if (bText) { /* RockDance 124 */
						ChangeSubItemText(nItem, nColumn, pStr, nPastePos);
					} else {
						ChangeSubItemText(nItem, nColumn, unquoteForComment(CString(pStr)), nPastePos);
					}
#ifdef FLICKERFREE
					InvalidateItemRect(nItem);
#endif
...
bool CMySuperGrid::ClipboardPaste(void)
{
	...
	if (hMem != NULL) {
		...
		if ((pBuffer = (LPSTR)GlobalLock(hMem)) != NULL) {
			...
			//SetRedraw(FALSE);
#endif
			PasteString(pBuffer, 0, bText); /* RockDance 124 */
#ifdef FLICKERFREE
			//SetRedraw(TRUE);
#endif
			GlobalUnlock(hMem);
...
bool CMySuperGrid::CellCopyDown(void)
{
	...
	for (int nColumn = sx; nColumn <= ex; nColumn++) {
		...
		if (pItemBase != NULL && IsItemFile(pItemBase) == TRUE) {
			...
			for (int nItem = sy+1; nItem <= ey; nItem++) {
				CTreeItem	*pItem = GetTreeItem(nItem);
				if (pItem != NULL && IsItemFile(pItem) == TRUE) {
					// セルに文字列を設定
#ifdef FLICKERFREE
...
int CMySuperGrid::CompFunc(CString &str1, CString &str2, int nColum, bool bSortAscending)
{
	...
	if (nColum == g_nColumnNumberList[COLUMN_TRACK_NUMBER] &&
			/* BeachMonster4 118 *///CId3tagv2::IsNumeric(str1) && CId3tagv2::IsNumeric(str2)) { /* BeachMonster 106 */
		...
		if (g_bOptSortIgnoreZenHan) {
			...
			if (!g_bOptSortIgnoreKataHira) { /* 2007.04.03 */
				strcpy(buffer1, str1);
				strcpy(buffer2, str2);
			}
		}
		...
		if (result == 0) { /* 2007.04.03 */
			if (g_bOptSortIgnoreKataHira) {
	...
	return result;
#ifdef BeachMonster4	/* BeachMonster4 114 */
	if (bSortAscending) {
		// 昇順ソート
		if (nColum == g_nColumnNumberList[COLUMN_TRACK_NUMBER]) {
			if (CId3tagv2::IsNumeric(str1) && CId3tagv2::IsNumeric(str2)) { /* BeachMonster 106 */
...
void CMySuperGrid::MultiColumnSort(SORT_STATE *sortState, int nKeys)
{
	...
	for (nIndex = 0; nIndex < nMax; nIndex++) {
		...
		if (pItem != NULL && IsItemFile(pItem) == TRUE) {
			...
			for (int nEnd = nIndex+1; nEnd < nMax; nEnd++) {
...
//				if (IsCurrentCellEditOK()) {
	...
	if (bUpdateMenuText2) {  /* TyphoonSwell 027 */
		...
		for (int i = 0; i < /*3*//*5*//* WildCherry 066 */sizeof(dwCmdID)/sizeof(UINT)/* STEP 030 */; i++) {
...
#if 0
	if (bUpdateMenuText || bUpdateMenuText2) {  /* RockDance 129 */
		UINT	dwCmdID[] = {
			ID_FAVORITE_FOLDER_01,
			ID_FAVORITE_FOLDER_02,
			ID_FAVORITE_FOLDER_03,
			ID_FAVORITE_FOLDER_04,
			ID_FAVORITE_FOLDER_05,
			ID_FAVORITE_FOLDER_06,
			ID_FAVORITE_FOLDER_07,
			ID_FAVORITE_FOLDER_08,
			ID_FAVORITE_FOLDER_09,
			ID_FAVORITE_FOLDER_10,
		};
		for (int i = 0; i < 10; i++) {
			// メニュー項目の書き換え
			CString	strName;
			// メニュー項目名を取得
			switch(dwCmdID[i]) {
			case ID_FAVORITE_FOLDER_01:
				strName.Format("01：%s", shortFavorite(g_strFavorite[0]));
				break;
			case ID_FAVORITE_FOLDER_02:
				strName.Format("02：%s", shortFavorite(g_strFavorite[1]));
				break;
			case ID_FAVORITE_FOLDER_03:
				strName.Format("03：%s", shortFavorite(g_strFavorite[2]));
				break;
			case ID_FAVORITE_FOLDER_04:
				strName.Format("04：%s", shortFavorite(g_strFavorite[3]));
				break;
			case ID_FAVORITE_FOLDER_05:
				strName.Format("05：%s", shortFavorite(g_strFavorite[4]));
				break;
			case ID_FAVORITE_FOLDER_06:
				strName.Format("06：%s", shortFavorite(g_strFavorite[5]));
				break;
			case ID_FAVORITE_FOLDER_07:
				strName.Format("07：%s", shortFavorite(g_strFavorite[6]));
				break;
			case ID_FAVORITE_FOLDER_08:
				strName.Format("08：%s", shortFavorite(g_strFavorite[7]));
				break;
			case ID_FAVORITE_FOLDER_09:
				strName.Format("09：%s", shortFavorite(g_strFavorite[8]));
				break;
			case ID_FAVORITE_FOLDER_10:
				strName.Format("10：%s", shortFavorite(g_strFavorite[9]));
				break;
			default:
				pPopup->GetMenuString((UINT)dwCmdID[i], strName, MF_BYCOMMAND);
				break;
...
void CMySuperGrid::CellStateControl(int nMode)
{
	...
	if (nMode == 0) {
		...
		for (nColumn = 0; nColumn < COLUMN_MAX; nColumn++) {
			...
			stat->nNumber = MyGetProfileInt(sSectionColumnState, strKey, stat->nNumber);
	...
	case 3:		// ユーザー指定の[タグ情報] => [ファイル名]変換
		{
			...
			strTrackNumber1.Format("%d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"))));
			...
			CString	strDiskNumber1; /* STEP 044 */
			CString	strDiskNumber2; /* STEP 044 */
			CString	strDiskNumber3; /* STEP 044 */
			strDiskNumber1.Format("%d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_DISK_NUMBER).SpanExcluding("\r")))); /* STEP 044 */
			strDiskNumber2.Format("%02d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_DISK_NUMBER).SpanExcluding("\r")))); /* STEP 044 */
			strDiskNumber3.Format("%03d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_DISK_NUMBER).SpanExcluding("\r")))); /* STEP 044 */
			...
			strFileName = StrReplace(strFileName, "%TRACK_NUMBER%" , GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"));
			strFileName = StrReplace(strFileName, "%TRACK_NUMBER1%", strTrackNumber1); /* STEP 046 */
			...
			strFileName = StrReplace(strFileName, "%ALBUM_ARTIST%", fileMP3->strAlbumArtist.SpanExcluding("\r")); /* STEP 042 */
			strFileName = StrReplace(strFileName, "%WRITER%"    , fileMP3->strWriter.SpanExcluding("\r")); /* STEP 043 */
			strFileName = StrReplace(strFileName, "%DISK_NUMBER%", fileMP3->strDiskNumberSI.SpanExcluding("\r")); /* STEP 045 */
			strFileName = StrReplace(strFileName, "%DISK_NUMBER1%", strDiskNumber1); /* STEP 045 */
			strFileName = StrReplace(strFileName, "%DISK_NUMBER2%", strDiskNumber2); /* STEP 045 */
			strFileName = StrReplace(strFileName, "%DISK_NUMBER3%", strDiskNumber3); /* STEP 045 */
			strFileName = StrReplace(strFileName, "%COMPILATION%", GetFileColumnText(fileMP3, COLUMN_COMPILATION).SpanExcluding("\r")); /* STEP 049 */
...
/* 2003.06.18 end */
			while(*sFormat != '\0') {
				...
				if (strnicmp(sFormat, "%TRACK_NAME%", 12) == 0) {
					nColumn = COLUMN_TRACK_NAME;
					nLen = 12;
				} else if (strnicmp(sFormat, "%ALBUM_NAME%", 12) == 0) {
					nColumn = COLUMN_ALBUM_NAME;
					nLen = 12;
				} else if (strnicmp(sFormat, "%ARTIST_NAME%", 13) == 0) {
					nColumn = COLUMN_ARTIST_NAME;
					nLen = 13;
				} else if (strnicmp(sFormat, "%TRACK_NUMBER%", 14) == 0) {
					nColumn = COLUMN_TRACK_NUMBER;
					nLen = 14;
				} else if (strnicmp(sFormat, "%TRACK_NUMBER1%", 15) == 0) { /* STEP 046 */
					nColumn = COLUMN_TRACK_NUMBER;
					nLen = 15;
				} else if (strnicmp(sFormat, "%TRACK_NUMBER2%", 15) == 0) {
					nColumn = COLUMN_TRACK_NUMBER;
					nLen = 15;
				} else if (strnicmp(sFormat, "%TRACK_NUMBER3%", 15) == 0) {
					nColumn = COLUMN_TRACK_NUMBER;
					nLen = 15;
				} else if (strnicmp(sFormat, "%RELEASE_YEAR%", 14) == 0) {
					nColumn = COLUMN_YEAR;
					nLen = 14;
				} else if (strnicmp(sFormat, "%COMMENT%", 9) == 0) {
					nColumn = COLUMN_COMMENT;
					nLen = 9;
				} else if (strnicmp(sFormat, "%GENRE%", 7) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_GENRE;
					nLen = 7;
				} else if (strnicmp(sFormat, "%COPYRIGHT%", 11) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_COPYRIGHT;
					nLen = 11;
				} else if (strnicmp(sFormat, "%ENGINEER%", 10) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_ENGINEER;
					nLen = 10;
				} else if (strnicmp(sFormat, "%SOURCE%", 8) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_SOURCE;
					nLen = 8;
				} else if (strnicmp(sFormat, "%SOFTWARE%", 10) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_SOFTWARE;
					nLen = 10;
				} else if (strnicmp(sFormat, "%KEYWORD%", 9) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_KEYWORD;
					nLen = 9;
				} else if (strnicmp(sFormat, "%TECHNICIAN%", 12) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_TECHNICIAN;
					nLen = 12;
				} else if (strnicmp(sFormat, "%LYRIC%", 7) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_LYRIC;
					nLen = 7;
				} else if (strnicmp(sFormat, "%COMMISSION%", 12) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_COMMISSION;
					nLen = 12;
				} else if (strnicmp(sFormat, "%COMPOSER%", 10) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_COMPOSER;
					nLen = 10;
				} else if (strnicmp(sFormat, "%ORIG_ARTIST%", 13) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_ORIG_ARTIST;
					nLen = 13;
				} else if (strnicmp(sFormat, "%URL%", 5) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_URL;
					nLen = 5;
				} else if (strnicmp(sFormat, "%ENCODEST%", 10) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_ENCODEST;
					nLen = 10;
				} else if (strnicmp(sFormat, "%OTHER%", 7) == 0) { /* Conspiracy 200 */
					nColumn = COLUMN_OTHER;
					nLen = 7;
				} else if (strnicmp(sFormat, "%ALBUM_ARTIST%", 14) == 0) { /* STEP 042 */
					nColumn = COLUMN_ALBUM_ARTIST;
					nLen = 14;
				} else if (strnicmp(sFormat, "%WRITER%", 8) == 0) { /* STEP 043 */
					nColumn = COLUMN_WRITER;
					nLen = 8;
				} else if (strnicmp(sFormat, "%DISK_NUMBER%", 13) == 0) { /* STEP 045 */
					nColumn = COLUMN_DISK_NUMBER;
					nLen = 8;
				} else if (strnicmp(sFormat, "%DISK_NUMBER1%", 14) == 0) { /* STEP 045 */
					nColumn = COLUMN_DISK_NUMBER;
					nLen = 8;
				} else if (strnicmp(sFormat, "%DISK_NUMBER2%", 14) == 0) { /* STEP 045 */
					nColumn = COLUMN_DISK_NUMBER;
					nLen = 8;
				} else if (strnicmp(sFormat, "%DISK_NUMBER3%", 14) == 0) { /* STEP 045 */
					nColumn = COLUMN_DISK_NUMBER;
					nLen = 8;
				} else if (strnicmp(sFormat, "%SKIP%", 6) == 0) {
					nColumn = COLUMN_DUMMY;
					nLen = 6;
				} else if (strnicmp(sFormat, "%COMPILATION%", 13) == 0) { /* STEP 049 */
...
//						if (strText.IsEmpty()) {
						if (CFileMP3::IsNumeric(strWord)) { /* STEP 021 */
							if(strnicmp(sFormat, "%TRACK_NUMBER%", 14) == 0) {
								strWord.Format("%d", atoi(strWord));
							}
							if(strnicmp(sFormat, "%TRACK_NUMBER1%", 15) == 0) { /* STEP 046 */
								strWord.Format("%d", atoi(strWord));
							}
							if(strnicmp(sFormat, "%TRACK_NUMBER2%", 15) == 0) {
								strWord.Format("%02d", atoi(strWord));
							}
							if(strnicmp(sFormat, "%TRACK_NUMBER3%", 15) == 0) {
								strWord.Format("%03d", atoi(strWord));
							}
							if(strnicmp(sFormat, "%DISK_NUMBER%", 13) == 0) { /* STEP 045 */
								strWord.Format("%d", atoi(strWord));
							}
							if(strnicmp(sFormat, "%DISK_NUMBER1%", 14) == 0) { /* STEP 045 */
								strWord.Format("%02d", atoi(strWord));
							}
							if(strnicmp(sFormat, "%DISK_NUMBER2%", 14) == 0) { /* STEP 046 */
								strWord.Format("%02d", atoi(strWord));
							}
							if(strnicmp(sFormat, "%DISK_NUMBER3%", 14) == 0) { /* STEP 046 */
								strWord.Format("%03d", atoi(strWord));
							}						}
	...
	if (pItem != NULL && IsItemFile(pItem) == TRUE) {
		...
		for (nTargetColumn = nStart; nTargetColumn <= nEnd; nTargetColumn++) {
			...
			case COLUMN_TRACK_NUMBER:	// トラック番号
				/* BeachMonster 105 */
				//nType = TYPE_NUMERIC;
				//dwData = (UINT)fileMP3->byTrackNumber;
...
bool CMySuperGrid::ConvUserFormatEx(USER_CONV_FORMAT_EX *pForm)
{
	...
	for (int nItem = sy; nItem <= ey; nItem++) {
		...
		if (pItem != NULL && IsItemFile(pItem) == TRUE) {
			...
			CString	strNumber, strTrackNumber2, strTrackNumber3;
			CString strTrackNumber1; /* STEP 046 */
			...
			strTrackNumber1.Format("%d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"))));
			// ２桁のトラック番号
			//strTrackNumber2.Format("%02d", atoi(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER)));
			strTrackNumber2.Format("%02d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"))));
			// ３桁のトラック番号
			strTrackNumber3.Format("%03d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"))));
			// １桁のディスク番号
			...
			strText = StrReplace(strText, "%TRACK_NUMBER%" , GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"));
			strText = StrReplace(strText, "%TRACK_NUMBER1%", strTrackNumber1); /* STEP 046 */
			...
			strText = StrReplace(strText, "%ALBUM_ARTIST%", fileMP3->strAlbumArtist.SpanExcluding("\r")); /* STEP 042 */
			strText = StrReplace(strText, "%WRITER%"    , fileMP3->strWriter.SpanExcluding("\r")); /* STEP 043 */
			strText = StrReplace(strText, "%DISK_NUMBER%", fileMP3->strDiskNumberSI.SpanExcluding("\r")); /* STEP 045 */
			strText = StrReplace(strText, "%DISK_NUMBER1%", strDiskNumber1); /* STEP 045 */
			strText = StrReplace(strText, "%DISK_NUMBER2%", strDiskNumber2); /* STEP 045 */
			strText = StrReplace(strText, "%DISK_NUMBER3%", strDiskNumber3); /* STEP 045 */
			strText = StrReplace(strText, "%COMPILATION%", GetFileColumnText(fileMP3, COLUMN_COMPILATION).SpanExcluding("\r")); /* STEP 049 */
			...
			for (int nColumn = sx; nColumn <= ex; nColumn++) {
				...
				if (g_nColumnTypeList[nColumn] == COLUMN_FILE_NAME) {
					...
					default:
						...
						/*
						&&	(strcmpi(ext, ".mp3") == 0 || strcmpi(ext, ".rmp") == 0 ||
				...
				if (strText.GetLength() > 0) {
					...
					case CONV_STR_KATA2HIRA:	// カタカナ => ひらがな変換
						...
						break;
#if 0
					case CONV_STR_KAN2HIRA:	// 漢字 => ひらがな変換
						{
							for (int i=0;;i++) {
								DWORD dwSize = strText.GetLength()*(i+2)+20;
								if (conv_kan2hira(GetSafeHwnd(), (unsigned char *)strText.GetBuffer(dwSize), dwSize) == 0)
									break;
							}
						}
						break;
					case CONV_STR_ROMAJI:	// ローマ字
						{
							for (int i=0;;i++) {
								DWORD dwSize = strText.GetLength()*(i+2)+20;
								if (conv_kan2hira(GetSafeHwnd(), (unsigned char *)strText.GetBuffer(dwSize), dwSize) == 0)
						...
						delete[]	buffer;
						buffer = new char[strText.GetLength()*3+2];
						conv_romaji(GetSafeHwnd(), (unsigned char *)strText.GetBuffer(0), (unsigned char*)buffer);
						strText = buffer;
						break;
#endif
					}
...
bool CMySuperGrid::WriteFormatFileBody(CFile &file, CTreeItem *pItem, const CString &strBody, bool bIsHtml, LIST_WRITE_STATUS *pStatus, bool bWriteHtml /* BeachMonster5 120 */)
{
	...
	if (pItem != NULL) {
		...
		if (IsItemFile(pItem) == TRUE) {
			...
			CString	strTrackNumber2;
			CString	strTrackNumber3;
			strTrackNumber3.Format("%03d", atoi(CId3tagv2::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER))));
			if ((nTrackNumber = atoi(/* BeachMonster 096 */CId3tagv2::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER)))) != 0) {
				strTrackNumber2.Format("%02d", nTrackNumber);
				strTrackNumber3.Format("%03d", atoi(/* BeachMonster 096 */CId3tagv2::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER))));
			}
			...
			strText = StrReplaceEx(strText, "%TRACK_NUMBER%"    , ConvHTML(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER), bWriteHtml)/* BeachMonster5 120 */, bIsHtml);
			...
			strText = StrReplaceEx(strText, "%LYRIC_FILE_NAME%" , ConvHTML(strLyricFile, bWriteHtml)/* BeachMonster5 120 */, bIsHtml);
			...
			strText = StrReplaceEx(strText, "%ALBUM_ARTIST%"	, ConvHTML(fileMP3->strAlbumArtist, bWriteHtml), bIsHtml); /* STEP 042 */
			strText = StrReplaceEx(strText, "%WRITER%"			, ConvHTML(fileMP3->strWriter, bWriteHtml), bIsHtml); /* STEP 043 */
			strText = StrReplaceEx(strText, "%DISK_NUMBER%"		, ConvHTML(fileMP3->strDiskNumberSI, bWriteHtml), bIsHtml); /* STEP 045 */
			strText = StrReplaceEx(strText, "%COMPILATION%"		, ConvHTML(GetFileColumnText(fileMP3, COLUMN_COMPILATION), bWriteHtml), bIsHtml); /* STEP 049 */

			/* Conspiracy 198 */
			if (fileMP3->nFormat != FILE_FORMAT_UNKNOWN) {
				// 総合演奏時間
				CString strBuffer;
				if (fileMP3->nPlayTime >= 0) { /* Conspiracy 195 */
					strBuffer.Format("%02d:%02d", fileMP3->nPlayTime/60, fileMP3->nPlayTime%60);
				...
				strText = StrReplace(strText, "%TOTAL_SEC2%", strBuffer, bIsHtml);
				// フォーマット
				strText = StrReplaceEx(strText, "%FORMAT", fileMP3->strFormat, bIsHtml);
				...
				// 総合演奏時間
				strText = StrReplaceEx(strText, "%TOTAL_SEC%", "", bIsHtml);
				// 総合演奏時間 /* STEP 020 */
				strText = StrReplaceEx(strText, "%TOTAL_SEC3%", "", bIsHtml);
				// 総合演奏時間(秒) /* FunnyCorn 172 */
				strText = StrReplaceEx(strText, "%TOTAL_SEC2%", "", bIsHtml);
				// フォーマット
				strText = StrReplaceEx(strText, "%FORMAT", "", bIsHtml);
			}
/* Conspiracy 198 */
//			// MPEG 情報を取得
//			MPEG_INFO	mpegInfo;
//			if (fileMP3->nFormat == FILE_FORMAT_APE || fileMP3->nFormat == FILE_FORMAT_WMA) { /* Misirlou 148 */
//				// 総合演奏時間
//				CString strBuffer;
//				if (fileMP3->nPlayTime >= 0) { /* Conspiracy 195 */
//					strBuffer.Format("%02d:%02d", fileMP3->nPlayTime/60, fileMP3->nPlayTime%60);
...
//				// 出力チャンネル数
//				strText = StrReplaceEx(strText, "%CHANNELS%", "", bIsHtml);
//				// ビットレート
//				strText = StrReplaceEx(strText, "%BIT_RATE%", "", bIsHtml);
//				// サンプリングレート
//				strText = StrReplaceEx(strText, "%SAMPLING_RATE%", "", bIsHtml);
//				// フレーム数
//				strText = StrReplaceEx(strText, "%FRAMES%", "", bIsHtml);
//				// チャンネルモード  /* BeachMonster 105 */
//				strText = StrReplaceEx(strText, "%CHANNEL_MODE%", "", bIsHtml);
//
//				if (fileMP3->nPlayTime >= 0) { /* Conspiracy 195 */
//					pStatus->nTotalTime += fileMP3->nPlayTime; /* Baja 163 */
//				}
//			} else
//			if (CFileMP3::GetMpegInfo(fileMP3->strFullPathName, &mpegInfo)) {
//				CString	strInfo;
//				// 出力チャンネル数
//				strInfo.Format("%d", mpegInfo.channels);
//				strText = StrReplace(strText, "%CHANNELS%", strInfo);
//				// ビットレート
//				strInfo.Format("%d", mpegInfo.bitRate);
//				strText = StrReplace(strText, "%BIT_RATE%", strInfo);
//				// サンプリングレート
//				strInfo.Format("%d", mpegInfo.samplingRate);
//				strText = StrReplace(strText, "%SAMPLING_RATE%", strInfo);
//				// フレーム数
//				strInfo.Format("%d", mpegInfo.flames);
//				strText = StrReplace(strText, "%FRAMES%", strInfo);
//				// チャンネルモード  /* BeachMonster 105 */
...
//					break;
//				}
//				strText = StrReplace(strText, "%CHANNEL_MODE%", strInfo);
//				// 総合演奏時間
//				strInfo.Format("%02d:%02d", mpegInfo.totalSec / 60, mpegInfo.totalSec % 60);
//				strText = StrReplace(strText, "%TOTAL_SEC%", strInfo);
//				// 総合演奏時間(秒) /* FunnyCorn 172 */
//				strInfo.Format("%d", mpegInfo.totalSec);
//				strText = StrReplaceEx(strText, "%TOTAL_SEC2%", strInfo, bIsHtml);
//
//				pStatus->nTotalTime += mpegInfo.totalSec;
//			} else {
//				// 出力チャンネル数
//				strText = StrReplaceEx(strText, "%CHANNELS%", "", bIsHtml);
//				// ビットレート
...
CString	CMySuperGrid::ReplaceFileName(const FILE_MP3 *fileMP3, CString strName)
{
	...
	*/
	...
	{// 文字を置換(ユーザ指定) /* FreeFall 050 */
		for (int i = 0; i < USER_FILENAME_REPLACE_MAX; i++) {
	...
	// 文字を置換
	for (int i = 0; i < FILENAME_REPLACE_MAX; i++) {
...
//	}
	...
	*/
	...
	CUIntArray arRemoveFile;
	for (int i=0;i<nCount;i++) {
		if (nOverWriteFlag == 3)	break;
		if(m_pDoc->GetDlgLoadProgressBarPtr()->IsCanceled()){ /* 2007.04.03 */
			//キャンセルボタンが押されていた
			break;
		}
		// プログレスバー更新
		...
		CString strDiskNumber1, strDiskNumber2, strDiskNumber3; /* STEP 045 */
		// 連番
		//strNumber = GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER);
		// １桁のトラック番号
		strTrackNumber1.Format("%d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r")))); /* STEP 046 */
		// ２桁のトラック番号
		//strTrackNumber2.Format("%02d", atoi(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER)));
		strTrackNumber2.Format("%02d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"))));
		// ３桁のトラック番号
		...
		strText = StrReplace(strText, "%GENRE%"        , GetFileColumnText(fileMP3, COLUMN_GENRE).SpanExcluding("\r"));
		//strText = StrReplace(strText, "%NUMBER%"       , strNumber);
		strText = StrReplace(strText, "%DISK_NUMBER%" , GetFileColumnText(fileMP3, COLUMN_DISK_NUMBER).SpanExcluding("\r")); /* STEP 045 */
		strText = StrReplace(strText, "%DISK_NUMBER1%", strDiskNumber1); /* STEP 045 */
		strText = StrReplace(strText, "%DISK_NUMBER2%", strDiskNumber2); /* STEP 045 */
		strText = StrReplace(strText, "%DISK_NUMBER3%", strDiskNumber3); /* STEP 045 */
		strText = StrReplace(strText, "%STRING%"       , pForm->strFixString);
		strText = StrReplace(strText, "%COMPILATION%" , GetFileColumnText(fileMP3, COLUMN_COMPILATION).SpanExcluding("\r")); /* STEP 049 */
		strText = StrReplace(strText, "%ALBUM_ARTIST%", GetFileColumnText(fileMP3, COLUMN_ALBUM_ARTIST).SpanExcluding("\r")); /* STEP 050 */
		...
		{// フォルダを作成して移動またはコピー
			...
			CDoubleZeroString strNewPaths; // for FOF_MULTIDESTFILES /* 2006.06.19 */
			CString strNewNameCollision; /* 2006.06.19 */
			...
			if (nOverWriteFlag != 1) { /* STEP 041 */
				...
				_tmakepath(sDestFile, NULL, strNewPath, fname, ext);
				if (GetFileAttributes(sDestFile) != 0xFFFFFFFF) {
					...
					case 4:
						...
						{ /* 2006.06.19 */
							TCHAR	drive[_MAX_DRIVE], dir[_MAX_DIR], fname[_MAX_FNAME], ext[_MAX_EXT];
							TCHAR	sCollisionFile[FILENAME_MAX+1];
							_tsplitpath(dialog.m_strFileName, drive, dir, fname, ext);
							_tmakepath(sCollisionFile, drive, dir, fname, NULL);
							strNewNameCollision = sCollisionFile;
						}
						sfo.pTo = strNewPath = dialog.m_strFileName;
						strNewPaths.Add(strNewPath);
						break;
					}
				}
			}
			strFile.Add(GetFileColumnText(fileMP3, COLUMN_FULL_PATH_NAME) + GetFileColumnText(fileMP3, COLUMN_FILE_NAME));
			{
				...
				for (int i = 0; i < 2; i++) {
					...
					if (GetFileAttributes(sLyricsFile) != 0xFFFFFFFF) {
						...
						if (strNewNameCollision.GetLength() > 0) {
							_tmakepath(sLyricsFile, NULL, NULL, strNewNameCollision, sLyricsExt);
							strNewPaths.Add(sLyricsFile);
						}
						// /* 2006.10.27 */sfo.fFlags |= FOF_MULTIDESTFILES;
					}
				}
			}
			// /* 2006.10.27 */if (strNewNameCollision.GetLength() > 0 && (sfo.fFlags & FOF_MULTIDESTFILES) == FOF_MULTIDESTFILES) { /* 2006.06.19 */
			if (strNewNameCollision.GetLength() > 0) { /* 2006.10.27 */
				sfo.pTo = strNewPaths;
				sfo.fFlags |= FOF_MULTIDESTFILES; /* 2006.10.27 */
			}
			sfo.pFrom = strFile;
			...
			if (::SHFileOperation(&sfo) == 0) {
				if ((sfo.fFlags & FOF_MULTIDESTFILES) == FOF_MULTIDESTFILES) { /* 2006.06.19 */
					strMoveFile.Add(strNewPath);
				} else {
					strMoveFile.Add(strNewPath + GetFileColumnText(fileMP3, COLUMN_FILE_NAME));
				}
				arRemoveFile.InsertAt(0, nIndex); /* 2006.10.27 */
			} else {
				break;
			}
		}
...
#ifdef mogtan
	{
		...
		nCount = MakeSelectFileArray(arrayList);
		int fileCount=0;
		for (int i = 0; i < nCount; i++, fileCount++) {
...
*/
		// 成功したものをリストから削除 /* 2006.10.27 */
		for (int i=0;i<arRemoveFile.GetSize();i++) {
			int		nIndex = arRemoveFile.GetAt(i);
			int		nNumber = (int)GetLParamFromIndex(nIndex);
			if (nNumber >= 0) {
				FILE_MP3	*fileMP3 = m_pDoc->GetListMP3(nNumber);
				UpdateParentAtDeleteItem(nIndex, fileMP3);
		...
		m_pDoc->UpdateAllViews(NULL);
		// 成功したものをリストに追加
		for (i=0;i<strMoveFile.GetSize();i++) {
...
void CMySuperGrid::UpdateParentAtDeleteItem(UINT nIndex, FILE_MP3 *fileMP3)
{
	...
	if (pItemParent != NULL) {
		CString strBuffer;
		int		nIndex = NodeToIndex(pItemParent);
		int		nColNum = g_nColumnNumberList[COLUMN_TRACK_NUMBER];
		if (nColNum >= 0) {
			strBuffer = pInfo->GetSubItem(nColNum-1);
			strBuffer.Format("%d", atoi(strBuffer)-1);
		...
		if (nColNum >= 0) {
			strBuffer = pInfo->GetSubItem(nColNum-1);
			strBuffer.TrimRight("MB");
			float size = atof(strBuffer) - (float)fileMP3->lFileSize / 1024 / 1024;
			if (size == 0.0) {
				strBuffer = "";
			} else {
				strBuffer.Format("%.2fMB", size);
			}
			pInfo->SetSubItemText(nColNum-1, strBuffer);
			SetItemText(nIndex, nColNum, strBuffer);
		}
		...
		if (m_nPrevTipItem != ht.iItem || m_nPrevTipSubItem != ht.iSubItem) {
			...
			if (strToolTip != "Root") {
				if (ItemHasChildren(pItem)) {
					...
					strToolTip += (_T("\n[ﾌｫｰﾏｯﾄ] ") + strComment);
					strToolTip += (_T("\n[ﾌｧｲﾙ] ") + GetFileColumnText(fileMP3, COLUMN_FILE_NAME));
...
#if 0 // 失敗
void CMySuperGrid::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) /* Baja 167 */
{
	// TODO: この位置にメッセージ ハンドラ用のコードを追加するかまたはデフォルトの処理を呼び出してください
	m_toolTip.Pop();
	CSuperGridCtrl::OnHScroll(nSBCode, nPos, pScrollBar);
}

void CMySuperGrid::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) /* Baja 167 */
{
	// TODO: この位置にメッセージ ハンドラ用のコードを追加するかまたはデフォルトの処理を呼び出してください
	m_toolTip.Pop();
	CSuperGridCtrl::OnVScroll(nSBCode, nPos, pScrollBar);
}
#endif
...
void CMySuperGrid::ClipboardCopyFormat(USER_COPY_FORMAT_FORMAT *pForm) /* FunnyCorn 175 */
{
	...
	*/
	...
	for (int i=0;i<nCount;i++) {
		...
		CString strDiskNumber1, strDiskNumber2, strDiskNumber3; /* STEP 045 */
		// 連番
		//strNumber = GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r");
		// １桁のトラック番号
		strTrackNumber1.Format("%02d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"))));
		// ２桁のトラック番号
		//strTrackNumber2.Format("%02d", atoi(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER)));
		strTrackNumber2.Format("%02d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER).SpanExcluding("\r"))));
		// ３桁のトラック番号
		...
		strText = StrReplace(strText, "%TRACK_NUMBER3%", strTrackNumber3);
		strText = StrReplace(strText, "%RELEASE_YEAR%" , GetFileColumnText(fileMP3, COLUMN_YEAR).SpanExcluding("\r"));
		strText = StrReplace(strText, "%COMMENT%"      , GetFileColumnText(fileMP3, COLUMN_COMMENT).SpanExcluding("\r"));
		strText = StrReplace(strText, "%GENRE%"        , GetFileColumnText(fileMP3, COLUMN_GENRE).SpanExcluding("\r"));
		strText = StrReplace(strText, "%DISK_NUMBER%" , GetFileColumnText(fileMP3, COLUMN_DISK_NUMBER).SpanExcluding("\r")); /* STEP 045 */
		strText = StrReplace(strText, "%DISK_NUMBER1%", strDiskNumber1); /* STEP 045 */
		strText = StrReplace(strText, "%DISK_NUMBER2%", strDiskNumber2); /* STEP 045 */
...
CString CMySuperGrid::MakeFormatFileBody(FILE_MP3	*fileMP3, const CString &strBody, bool bIsHtml, LIST_WRITE_STATUS *pStatus, bool bWriteHtml)
{
	...
	if ((nTrackNumber = atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER)))) != 0) {
		strTrackNumber1.Format("%d", nTrackNumber); /* STEP 046 */
		strTrackNumber2.Format("%02d", nTrackNumber);
		//strTrackNumber3.Format("%03d", atoi(CFileMP3::GetIntTrackNo(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER))));
	...
	strText = StrReplaceEx(strText, "%ARTIST_NAME%"     , ConvHTML(GetFileColumnText(fileMP3, COLUMN_ARTIST_NAME), bWriteHtml)/* BeachMonster5 120 */, bIsHtml);
	strText = StrReplaceEx(strText, "%TRACK_NUMBER%"    , ConvHTML(GetFileColumnText(fileMP3, COLUMN_TRACK_NUMBER), bWriteHtml)/* BeachMonster5 120 */, bIsHtml);
	strText = StrReplaceEx(strText, "%TRACK_NUMBER1%"   , strTrackNumber1, bIsHtml); /* STEP 046 */
	strText = StrReplaceEx(strText, "%TRACK_NUMBER2%"   , strTrackNumber2, bIsHtml);
	strText = StrReplaceEx(strText, "%TRACK_NUMBER3%"   , strTrackNumber3, bIsHtml);
	strText = StrReplaceEx(strText, "%RELEASE_YEAR%"    , ConvHTML(GetFileColumnText(fileMP3, COLUMN_YEAR), bWriteHtml)/* BeachMonster5 120 */, bIsHtml);
	strText = StrReplaceEx(strText, "%COMMENT%"         , ConvHTML(GetFileColumnText(fileMP3, COLUMN_COMMENT), bWriteHtml)/* BeachMonster5 120 */, bIsHtml);
	...
	strText = StrReplaceEx(strText, "%WRITER%"			, ConvHTML(fileMP3->strWriter, bWriteHtml), bIsHtml); /* STEP 043 */
	strText = StrReplaceEx(strText, "%DISK_NUMBER%"		, ConvHTML(fileMP3->strDiskNumberSI, bWriteHtml), bIsHtml); /* STEP 045 */
	strText = StrReplaceEx(strText, "%DISK_NUMBER1%"   , strDiskNumber1, bIsHtml); /* STEP 045 */
	strText = StrReplaceEx(strText, "%DISK_NUMBER2%"   , strDiskNumber2, bIsHtml); /* STEP 045 */
	strText = StrReplaceEx(strText, "%DISK_NUMBER3%"   , strDiskNumber3, bIsHtml); /* STEP 045 */
	strText = StrReplaceEx(strText, "%COMPILATION%"		, ConvHTML(GetFileColumnText(fileMP3, COLUMN_COMPILATION), bWriteHtml), bIsHtml); /* STEP 049 */
